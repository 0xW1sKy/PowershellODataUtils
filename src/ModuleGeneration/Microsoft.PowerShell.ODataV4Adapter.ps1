...
function SaveCDXMLRemoveCmdlet {
    ...
    $navigationPrperties | Where-Object { $_ -ne $null } | ForEach-Object {
            ...
            if ($entityType -ne $null) {
                ...
                if ($entityType.Alias -ne $null -and $entityType.Alias -ne "") {
                    ...
                    if (!$complexTypeMapping.ContainsKey($entityTypeShortName)) {
                        $valueToAdd = $entityType.Namespace + '.' + $entityType.Name
                        if ($normalizedNamespaces.ContainsKey($entityType.Namespace)) {
                            $valueToAdd = $normalizedNamespaces[$entityType.Namespace] + '.' + $entityType.Name
                        }
                        $complexTypeMapping.Add($entityTypeShortName, $valueToAdd)
                    }
                }
            }
        }
        ...
        foreach ($enumType in $enumTypesToBeGenerated) {
            if ($enumType -ne $null) {
                ...
                if (!$complexTypeMapping.ContainsKey($enumTypeFullName)) {
                    $valueToAdd = $enumTypeFullName
                    if ($normalizedNamespaces.ContainsKey($enumType.Namespace)) {
                        $valueToAdd = $normalizedNamespaces[$enumType.Namespace] + '.' + $enumType.Name
                    }
                    $complexTypeMapping.Add($enumTypeFullName, $valueToAdd)
                }
                ...
                if (($enumType.Alias -ne $null -and $enumType.Alias -ne "") -or ($metadata.Alias -ne $null -and $metadata.Alias -ne "")) {
                    ...
                    if (!$complexTypeMapping.ContainsKey($enumTypeShortName)) {
                        ...
                        $complexTypeMapping.Add($enumTypeShortName, $valueToAdd)
                    }
                }
            }
        }
